...
var _chat = {
    ...
    _func_sendGift: function (data) {
        ...
        if(data.giftId==84||data.giftId==79||data.giftId==73||data.giftId==85||data.giftId==129){
    ...
    _onlineList:function(data){
        ...
        if(data.AI){data.client_name=data.nickname;}
        if(srcImg){
    ...
    _func_logout: function(data) {
        console.log(data);
        if(Anchor.id==data.user_id){
            GameSocketIO._gameOver();
